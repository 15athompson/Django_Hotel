...
class RoomManagementAcceptanceTest(UserAcceptanceTestCase):
    ...
    def test_room_reservation_status_management(self):
        ...
        """
        # 1. Create a reservation for a specific date
        # Use a fixed date to avoid test flakiness
        base_date = date(2025, 3, 1)  # March 1st, 2025
        reservation = Reservation.objects.create(
            guest=self.guest,
            room_number=self.standard_room,
            reservation_date_time=timezone.now(),
        ...
        response = self.client.post(
            reverse('reservation_update', args=[reservation.reservation_id]),
            {
                ...
                'amount_paid': '100.00',
                'number_of_guests': 2,
                'start_of_stay': base_date.strftime('%Y-%m-%d'),
                'length_of_stay': 1,
        ...
        # 3. Verify room doesn't appear in availability search for today
        response = self.client.get(
            reverse('available_rooms_list'),
            {
                'start_date': base_date.strftime('%Y-%m-%d'),
                'length_of_stay': 1,
                'room_type': 'STD'  # Explicitly search for standard rooms
            }
        )
        ...
        # 5. Verify room appears in availability search for the next day
        next_date = date(2025, 3, 2)  # March 2nd, 2025
        response = self.client.get(
            reverse('available_rooms_list'),
            {
